...
- **敌人**: slime/bat(迟到怪,schoolRush 冲刺)/note(传纸条,laneHop 换路)/doodle(涂鸦怪,doodleHeal 回血)/tank(作业山,paperShield 前半减伤)/boss(监考怪,focusDrain 红笔点名)/elf。
- **focusDrain 红笔点名**: 已完整实作（index.html boss 段：预警一路→伤害最前防御，班主任定身可化解；gated on isBoss + 场上有 boss）。
...
- **装备工坊**: `TOOL_UPGRADES`（6 件各 1 永久升级，铅笔→原子笔机枪等）；碎片 `shards` 购买；部署时 apply override（复用 rateOverride/atkOverride/slowDurOverride/maxHp；teacher 改 recharge）。
...
### 存档 schema（`xueba_pvz_save_v1`，payload schemaVersion:2）
`{ schemaVersion, unlockedLevel, hp, results{id:{stars,bestHp,bestWrong}}, stickers{}, badges{}, worldProgress{}, shards, toolUpgrades{} }`
...
### 不可破坏的硬约束
单档不拆、不引框架、不用 PvZ 素材；橫屏锁定/safe-area/visualViewport；存档导入导出向后相容（新字段必须同步 4 处：loadGameSave 默认 + migrate + export + import 白名单）。验证三件套：inline script syntax、`<script>`=1、`git diff --check`。
